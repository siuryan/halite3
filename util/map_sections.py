            ...
            for section_x in range(int(start_x), int(end_x)):
                for section_y in range(int(start_y), int(end_y)):
                    if x > 0 and y > 0 and ship.id % 4 == 0:
                        ...
                        section_values[x][y] += 10
                    section_values[x][y] += 1.0 * (game_map[Position(section_x, section_y)].halite_amount) / (game_map.calculate_distance(ship.position, Position(section_x, section_y)) + 1)**(1.005**(500-turns))
